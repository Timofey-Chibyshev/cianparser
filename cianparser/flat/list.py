...
def init_selenium():
    options = Options()
    options.add_argument("--headless")  # Запуск без отображения браузера
...
class FlatListPageParser(BaseListPageParser):
    def make_request_with_user_agent_rotation(self, url, max_attempts=5):
        ...
        while attempts < max_attempts:
            try:
                ...
                if response.status_code == 429:
                    ...
                    return response.text  # Возвращаем HTML страницы
        ...
        # Если после всех попыток загрузка не удалась, используем Selenium
        print("Using Selenium to load the page after failed attempts with requests...")
        driver = init_selenium()
        driver.get(url)
        time.sleep(random.uniform(5, 10))  # Задержка для полной загрузки страницы
